...
socket_ftp_download_file_speed = 100
            ...
            if ret[0] :
                while download_file_num < 10000:
                    ...
                    if ret[0] :
                        ...
                        scp_fd = ssh_scp.ssh_scp(self.scp_server_ip, 22, self.scp_user, self.scp_password,
    ...
    def handle_client_recvdata(self, data):
        ...
        for recv_data_per in cut_data :
            if 'getusername' in recv_data_per :
                ...
                if scp_fd.scp_file_dst_to_local(os.path.split(os.path.split(code_path)[0])[1]):
                    self.client_send(socket_client_fd, "updatecodeok:" + socket_client_fd + '*')
                    socket_client_fd.close()
                    cur_pid = os.getpid()
                    print_string = "update code ok"
                    print_info.print_info(print_info.PRINT_DEBUG, print_string)
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    if not data:
                        break
                    self.handle_client_recvdata(data)
                except Exception, e:
