...
class download_ftp(FTP):
        ...
        for n in range(3):
            try:
                ...
                try:
                    ...
                    print_info.print_info(print_info.PRINT_INFO, 'login ftp server success')
    ...
    def get_ftp_filesize(self, fd, remotefilepatch, remotefilename, bugid):
        ...
        if change_ret[0] :
            try :
                print 'fd :%s ;dir list :%s' % (fd, fd.dir)
                dir_list = fd.nlst()
            ...
            for x in dir_list:
                if remotefilename :
                    ...
                    try:
                        ...
                        print_info.print_info(print_info.PRINT_INFO, print_string)
            #ftp_file_list form [file name, file size]
            return (1, ftp_file_list)
...
if __name__ == '__main__':
    ...
    file_list = ftp_fd.get_ftp_filesize(ftp_fd, '/Testlogs/FAETest/Bugzilla_log/Bug 831329', '103.7z', 831329)
    #file_list = (1, ['103.7z' , 2000000])

    if file_list[0] :
        list = file_list[1]
        while i < len(list):
            ftp_fd.download_ftp_file_size(ftp_fd, '/Testlogs/FAETest/Bugzilla_log/Bug 831329', list[0],
                                         '/home/local/SPREADTRUM/lc.fan/flc/code/pybz', 831329, list[0], 0, list[1])
            i += 1;
    ...
    if file_list[0]:
        ...
        while i < len(list):
            ftp_fd.download_ftp_file_size(ftp_fd, '/Testlogs/PSST/KaiOS/9820E/SPRDROID6.0_KAIOS_17D_W18.03.2',
                                          list[0], '/home/local/SPREADTRUM/lc.fan/flc/code/pybz', 821366,
                                          list[0], 0, list[1])
            i += 1;
